import random

# AES parameter sets
AES_CONFIGS = {
    "AES-128": {"NB": 4, "NK": 4, "NR": 10},
    "AES-192": {"NB": 4, "NK": 6, "NR": 12},
    "AES-256": {"NB": 4, "NK": 8, "NR": 14},
}

# Current implementation variant
AES_VARIANT = "AES-128"

NB = AES_CONFIGS[AES_VARIANT]["NB"]
NK = AES_CONFIGS[AES_VARIANT]["NK"]
NR = AES_CONFIGS[AES_VARIANT]["NR"]

S_BOX = [
    
]

INV_S_BOX = [
    
]

RCON = [0x00, 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36]


def get_aes_variant():
    return AES_VARIANT


def auto_generate_aes_key():
    return "".join(random.choice("0123456789ABCDEF") for _ in range(32))


def text_to_bytes(text):
    return [ord(c) for c in text]


def bytes_to_text(byte_list):
    return "".join(chr(b) for b in byte_list)


def hex_to_bytes(hex_str):
    return list(bytes.fromhex(hex_str))


def bytes_to_hex(byte_list):
    return "".join(f"{b:02X}" for b in byte_list)


def pad_text(text):
    pad_len = 16 - (len(text) % 16)
    if pad_len == 0:
        pad_len = 16
    return text + chr(pad_len) * pad_len


def unpad_text(text):
    if not text:
        return text
    pad_len = ord(text[-1])
    if 1 <= pad_len <= 16 and text.endswith(chr(pad_len) * pad_len):
        return text[:-pad_len]
    return text


def split_blocks(text, size=16):
    return [text[i:i + size] for i in range(0, len(text), size)]


def bytes_to_state(block):
    state = [[0] * 4 for _ in range(4)]
    for i in range(16):
        state[i % 4][i // 4] = block[i]
    return state


def state_to_bytes(state):
    result = []
    for c in range(4):
        for r in range(4):
            result.append(state[r][c])
    return result


def sub_word(word):
    return [S_BOX[b] for b in word]


def rot_word(word):
    return word[1:] + word[:1]


def xor_words(a, b):
    return [x ^ y for x, y in zip(a, b)]


def key_expansion(key_hex):
    key_bytes = hex_to_bytes(key_hex)
    words = []

    for i in range(NK):
        words.append(key_bytes[4 * i: 4 * i + 4])

    for i in range(NK, NB * (NR + 1)):
        temp = words[i - 1][:]

        if i % NK == 0:
            temp = sub_word(rot_word(temp))
            temp[0] ^= RCON[i // NK]

        words.append(xor_words(words[i - NK], temp))

    round_keys = []
    for r in range(NR + 1):
        round_key = []
        for c in range(4):
            round_key.extend(words[r * 4 + c])
        round_keys.append(round_key)

    return words, round_keys


def add_round_key(state, round_key):
    for c in range(4):
        for r in range(4):
            state[r][c] ^= round_key[c * 4 + r]
    return state


def sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = S_BOX[state[r][c]]
    return state


def inv_sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = INV_S_BOX[state[r][c]]
    return state


def shift_rows(state):
    for r in range(1, 4):
        state[r] = state[r][r:] + state[r][:r]
    return state


def inv_shift_rows(state):
    for r in range(1, 4):
        state[r] = state[r][-r:] + state[r][:-r]
    return state


def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a = (a << 1) & 0xFF
        if hi_bit_set:
            a ^= 0x1B
        b >>= 1
    return p


def mix_single_column(col):
    a0, a1, a2, a3 = col
    return [
        gmul(a0, 2) ^ gmul(a1, 3) ^ a2 ^ a3,
        a0 ^ gmul(a1, 2) ^ gmul(a2, 3) ^ a3,
        a0 ^ a1 ^ gmul(a2, 2) ^ gmul(a3, 3),
        gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2),
    ]


def inv_mix_single_column(col):
    a0, a1, a2, a3 = col
    return [
        gmul(a0, 14) ^ gmul(a1, 11) ^ gmul(a2, 13) ^ gmul(a3, 9),
        gmul(a0, 9) ^ gmul(a1, 14) ^ gmul(a2, 11) ^ gmul(a3, 13),
        gmul(a0, 13) ^ gmul(a1, 9) ^ gmul(a2, 14) ^ gmul(a3, 11),
        gmul(a0, 11) ^ gmul(a1, 13) ^ gmul(a2, 9) ^ gmul(a3, 14),
    ]


def mix_columns(state):
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        mixed = mix_single_column(col)
        for r in range(4):
            state[r][c] = mixed[r]
    return state


def inv_mix_columns(state):
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        mixed = inv_mix_single_column(col)
        for r in range(4):
            state[r][c] = mixed[r]
    return state


def aes_encrypt_block(block_text, key_hex):
    if len(block_text) != 16:
        raise ValueError("AES block must be exactly 16 characters")

    _, round_keys = key_expansion(key_hex)

    block_bytes = text_to_bytes(block_text)
    state = bytes_to_state(block_bytes)

    state = add_round_key(state, round_keys[0])

    for rnd in range(1, NR):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[rnd])

    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[NR])

    return bytes_to_hex(state_to_bytes(state))


def aes_decrypt_block(block_hex, key_hex):
    if len(block_hex) != 32:
        raise ValueError("AES ciphertext block must be exactly 32 hex characters")

    _, round_keys = key_expansion(key_hex)

    block_bytes = hex_to_bytes(block_hex)
    state = bytes_to_state(block_bytes)

    state = add_round_key(state, round_keys[NR])

    for rnd in range(NR - 1, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, round_keys[rnd])
        state = inv_mix_columns(state)

    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, round_keys[0])

    return bytes_to_text(state_to_bytes(state))


def aes_encrypt_long_text(plaintext, key_hex):
    padded = pad_text(plaintext)
    blocks = split_blocks(padded, 16)

    cipher_blocks = []
    for block in blocks:
        cipher_blocks.append(aes_encrypt_block(block, key_hex))

    return " ".join(cipher_blocks)


def aes_decrypt_long_text(ciphertext, key_hex):
    blocks = ciphertext.strip().split()
    plaintext_blocks = []

    for block in blocks:
        plaintext_blocks.append(aes_decrypt_block(block, key_hex))

    return unpad_text("".join(plaintext_blocks))


def get_round_keys_hex(key_hex):
    _, round_keys = key_expansion(key_hex)
    return [bytes_to_hex(rk) for rk in round_keys]